...
Vervolgens overlopen we hoe Angular gebruik maakt van de TypeScript-programmeertaal.
...
% Wat is TypeScript?
Zoals omschreven door \textcite{Bierman2014} is TypeScript een programmeertaal ontwikkeld door Microsoft in 2012.
...
Volgens \textcite{Ramos2024} zijn er twee verschillende manieren waarop een Angular applicatie gecompileerd kan worden.
...
De syntax voor property binding is \mintinline{ts}{[ ]}, bijvoorbeeld:\\ \mintinline{html+ng2}{<button [disabled]="hasErrors()">Save</button>}.
\medskip
...
Two-way binding in Angular is hetzelfde als property binding.
...
Dit onderzoek gaat ervan uit dat het Jasmine-testing-framework gebruikt wordt sinds dit de standaard is.
\medskip
...
\begin{itemize}
  ...
  \item In v17 worden conditionele uitdrukkingen in templates strikter geëvalueerd. De werking moet overal nagegaan worden. 
    Dit kan redelijk wat impact hebben op de user interface.
...
Versies zijn niet gebonden aan het aantal of de samenhorigheid van de aanpassingen, zoals bij andere frameworks het geval is.
...
Om de betrouwbaarheid van onze applicatie te kunnen garanderen is het dus van belang om tijdig over te schakelen naar de nieuwste versie.
...
Voordat we hierop kunnen antwoorden, moeten we eerst het concept van \emph{code aanpassen} beter definiëren.
...
Dezelfde bron noemt AST's een fundamentele eigenschap van code.
